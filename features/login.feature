...
Feature: Login to the application
  ...
  Scenario Outline: Verify users can not login with invalid credentials
    When user enters invalid username '<user-name>'
    And user enters invalid password '<password>'
    And user clicks the login button
    Then users should see a "Sorry, Wrong Email or Password" error pop-up message
    Examples:
      | user-name      | password      |
      | admin@cydeo    | wrong         |
      | notAdmin@cydeo | wrongPassword |
  ...
  Scenario: Verify users are not allowed to login without any credentials
    ...
    Then users should see a "This field is required." error pop-up message
